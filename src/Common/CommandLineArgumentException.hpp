...
class CommandLineArgumentException : public std::runtime_error
{
  ...
  public:
    ...
    CommandLineArgumentException(std::string parameter) : std::runtime_error("")
    {
    ...
    CommandLineArgumentException(std::string parameter, std::string token) : std::runtime_error("")
    {
        std::ostringstream stringStream;
        stringStream << "Cannot match with expected parameters and flags: ";
        stringStream << parameter;
    ...
    {
        return exceptionMessaage.c_str();
    }
};
}
}
