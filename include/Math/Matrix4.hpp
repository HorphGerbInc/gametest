...
class Matrix4
{
  public:
    ...
    {
        jerobins::common::ArgumentCheck<uint8_t>(row <= 3, "Row is out of range", row);
        return data[row];
    }
    ...
    {
        Matrix4 result(*this);
        _MM_TRANSPOSE4_PS(result.data[0].xmm_, result.data[1].xmm_, result.data[2].xmm_, result.data[3].xmm_);
        return result;
    }
